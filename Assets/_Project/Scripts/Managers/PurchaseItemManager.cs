...
namespace M
{
    public class PurchaseItemManager : MonoBehaviour
    {
        ...
        [Header("Item UI Elements")] [SerializeField]
        ...
        [Header("Transaction Info")] [SerializeField]
        private TextMeshProUGUI transactionInfoText;
        ...
        private async Task<string> PurchaseItemFromContract(BigInteger tokenId, string metadataUrl)
        {
            ...
            string resp = await Moralis.ExecuteContractFunction(GameManager.ContractAddress, GameManager.ContractAbi,
                "buyItem", parameters, value, gas, gasPrice);
        ...
        {
            // 1. Build Metadata
            object metadata = MoralisTools.BuildMetadata(_currentItemData.name, _currentItemData.description,
                _currentItemData.imageUrl);
